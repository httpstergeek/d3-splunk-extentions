    ...
    DashboardController.onReady(function() {
        ...
        DashboardController.onViewModelLoad(function() {
            ...
            var exampleInfoLoaded = exampleInfoCollection.fetch({
                url: SplunkUtil.make_url('/static/app/' + app +'/getting_started.json'),
                cache: true
            });
            ...
            $.when(exampleInfoLoaded, dashboardsLoaded).then(function(){
                ...
                $('.dashboard-body').append($('<div class="row contents-body"></div>').append($('<div class="nav-bar-slide"></div>').append($nav)).append($contents).append($('<div class="feedback-bar"></div>').append($('<p id="helptext"></p>').html('Don\'t see something or need more help?  Email').css({'width':'200px','position':'absolute','left':'1220px','top': '30px','font-size': 'medium'}))));
                $nav.affix({
